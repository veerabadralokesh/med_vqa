...
def vqarad_process(args, clip_model, processor, data='train'):
    clip_model.to(device)

    dataset = datasets.load_dataset(data_urls[args.dataset_type])
    
    images = dataset[data]['image']
    questions = dataset[data]['question']
    answers = dataset[data]['answer']
    ...
    for i in tqdm(range(len(dataset['train']))): 
        ...
        text = '[quest] ' + questions[i] + ' [ans] ' + answers[i][0]
...
def main(): 
    ...
    # parser.add_argument('--clip_path', default='openai/clip-vit-base-patch32')
    parser.add_argument('--clip_path', default='openai/clip-vit-large-patch14')
    parser.add_argument('--dataset_path', default='../COCO')
    parser.add_argument('--dataset_type', default='VQA-RAD') 
    ...
    elif args.dataset_type == 'VQA-RAD':
        vqarad_process(args, clip_model, processor) 
    else:
        pass 
